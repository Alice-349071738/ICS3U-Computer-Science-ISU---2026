            ...
            if (userInput < 1 || userInput > 8) {
                ...
                while (subMenuLoop) {
                    ...
                    if (subMenuInput < 1 || subMenuInput > 3) { //Check for invalid input for sub menu
                        ...
                        if (title.size() <= 0) { //Checks if there are songs to remove in the ArrayList

                            System.out.println("There are no songs to remove. There are no songs to edit. Please add songs in the main menu.");
                            subMenuLoop = false; //Breaks loop, returns to main menu
                ...
                int indexOfBottom20 = (int)(sortRating.size() * 0.20); //Find bottom 20 percent, n x 0.2 = 20% of n
                int indexOfTop20 = sortRating.size() - (int)(sortRating.size() * 0.20); //Find top 20 percent, n - 20% of n = 80% which is the cutoff for the top 20%

                int bottomStop = sortRating.get(indexOfBottom20 - 1); //Find bottomCutoff by matching number with index since 1 song = index 0
                int topStop = sortRating.get(indexOfTop20);

                System.out.println("\nThe songs in the top 20% of ratings are:");
                for (int i = 0; i < rating.size(); i++) { //For loop with if statement to check if ratings in ArrayList are greater than cutoff for top 20%
                    if (sortRating.get(i) >= topStop) {
                        System.out.println("Title: " + sortTitle.get(i)+ "\nArtist: " + sortArtist.get(i) + "\nRating: " + sortRating.get(i) + "\n");
                    }
                }

                System.out.println("The songs in the bottom 20% of ratings are:");
                for (int i = 0; i < rating.size(); i++) { //For loop with if statement to check if ratings in ArrayList are less than cutoff for bottom 20%
                    if (sortRating.get(i) <= bottomStop) {
                        System.out.println("Title: " + sortTitle.get(i)+ "\nArtist: " + sortArtist.get(i) + "\nRating: " + sortRating.get(i) + "\n");
                    }
                }
